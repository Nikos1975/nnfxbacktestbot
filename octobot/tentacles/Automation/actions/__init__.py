...
if check_tentacle_version('1.2.0', 'stop_strategies_and_pause_trader_action', 'OctoBot-Default-Tentacles'):
    try:
        from .stop_strategies_and_pause_trader_action import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading stop_strategies_and_pause_trader_action: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
