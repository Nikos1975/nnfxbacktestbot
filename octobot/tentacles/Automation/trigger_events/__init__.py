...
if check_tentacle_version('1.2.0', 'volatility_threshold_event', 'OctoBot-Default-Tentacles'):
    try:
        from .volatility_threshold_event import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading volatility_threshold_event: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')

if check_tentacle_version('1.2.0', 'holding_threshold_event', 'OctoBot-Default-Tentacles'):
    try:
        from .holding_threshold_event import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading holding_threshold_event: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
