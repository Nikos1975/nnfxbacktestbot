...
if check_tentacle_version('1.2.0', 'lbank_websocket_feed', 'OctoBot-Default-Tentacles'):
    try:
        from .lbank_websocket_feed import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading lbank_websocket_feed: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
...
if check_tentacle_version('1.2.0', 'lbank', 'OctoBot-Default-Tentacles'):
    try:
        from .lbank import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading lbank: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
