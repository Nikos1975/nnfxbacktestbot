...
if check_tentacle_version('1.2.0', 'ai_trading_mode', 'OctoBot-Default-Tentacles'):
    try:
        from .ai_trading_mode import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading ai_trading_mode: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
...
if check_tentacle_version('1.2.0', 'profile_copy_trading_mode', 'OctoBot-Default-Tentacles'):
    try:
        from .profile_copy_trading_mode import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading profile_copy_trading_mode: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
...
if check_tentacle_version('1.2.0', 'dsl_trading_mode', 'OctoBot-Default-Tentacles'):
    try:
        from .dsl_trading_mode import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading dsl_trading_mode: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
