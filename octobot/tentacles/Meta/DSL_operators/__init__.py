...
if check_tentacle_version('1.2.0', 'blockchain_wallet_operators', 'OctoBot-Default-Tentacles'):
    try:
        from .blockchain_wallet_operators import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading blockchain_wallet_operators: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')

if check_tentacle_version('1.2.0', 'automation_operators', 'OctoBot-Default-Tentacles'):
    try:
        from .automation_operators import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading automation_operators: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
