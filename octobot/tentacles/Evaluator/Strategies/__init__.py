...
if check_tentacle_version('1.3.0', 'ai_strategies_evaluator', 'OctoBot-Default-Tentacles'):
    try:
        from .ai_strategies_evaluator import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading ai_strategies_evaluator: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
