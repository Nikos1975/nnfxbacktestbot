...
def infer_strategy_name(export_file: Path, data: Any) -> str:
    if isinstance(data, dict):
        ...
        for key in data.keys():
            if str(key).startswith("Nnfx"):
                return str(key)
    return export_file.stem.replace("-1d-MAX", "")
...
def find_trade_lists(obj: Any) -> list[list[dict[str, Any]]]:
    ...
    if isinstance(obj, list) and obj and all(isinstance(x, dict) for x in obj):
        ...
        if {"pair"} & keys and ({"profit_ratio", "profit_abs", "close_profit_abs"} & keys):
            found.append(obj)
...
def load_trades(export_file: Path) -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "strategy": infer_strategy_name(export_file, data),
                    "pair": t.get("pair"),
                    "open_date": parse_dt(
                        t.get("open_date")
...
def download_binance_daily(pair: str, start: str, end: str) -> pd.DataFrame:
    ...
    while current < end_ms:
        ...
        current = last_open + 24 * 60 * 60 * 1000
...
def main() -> int:
    ...
    parser.add_argument("--exports", required=True, help="Folder containing Freqtrade exported JSON backtest files.")
    ...
    files = sorted(export_dir.glob("*MAX*.json"))
    if not files:
        files = sorted(export_dir.glob("*.json"))

    if not files:
        print(f"No JSON export files found in {export_dir}", file=sys.stderr)
        return 2
    ...
    if not all_trades:
        print("No trades found in exported files.", file=sys.stderr)
        return 3
