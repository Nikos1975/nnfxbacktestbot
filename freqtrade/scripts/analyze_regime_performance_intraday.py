...
def infer_strategy_name(source_name: str, data: Any) -> str:
    ...
    return stem.replace("-1h-MAX", "").replace("-4h-MAX", "").replace("-1d-MAX", "")
...
def load_trades_from_results(paths: list[Path], timeframe: str) -> pd.DataFrame:
...
def main() -> int:
    ...
    token = f"-{args.timeframe}-MAX"
    paths = sorted(export_dir.glob(f"*{token}*.zip")) + sorted(export_dir.glob(f"*{token}*.json"))

    if not paths:
        print(f"No result files matching *{token}*.zip/json found in {export_dir}", file=sys.stderr)
        return 2

    trades = load_trades_from_results(paths, args.timeframe)

    if trades.empty:
        print("No trades found in exported result files.", file=sys.stderr)
        print("Check that backtests were run with --export trades.", file=sys.stderr)
        return 3
