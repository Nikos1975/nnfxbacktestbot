...
class NnfxMacZVwapOttVoxiVolStop(IStrategy):
    """
    MAC-Z VWAP proxy -> OTT state, with VOXI proxy and Volatility Stop.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        support, ott_line = self._ott_proxy(close)
        dataframe["ott_support"] = support
        dataframe["ott_line"] = ott_line
        dataframe["macz_vwap"] = self._mac_z_vwap_proxy(dataframe)
        dataframe["voxi"] = self._voxi_proxy(dataframe)
        atr = self._atr_rma(dataframe, 20)
        dataframe["vol_stop"] = close.rolling(20, min_periods=20).max() - 2.0 * atr
        dataframe["baseline_signal"] = support > ott_line
        dataframe["c1_signal"] = dataframe["macz_vwap"] > 0
        dataframe["c2_signal"] = support > ott_line
        dataframe["volume_filter"] = dataframe["voxi"] > 1.0
        dataframe["exit_signal"] = (close < dataframe["vol_stop"]) | (dataframe["macz_vwap"] < 0)
        return dataframe
