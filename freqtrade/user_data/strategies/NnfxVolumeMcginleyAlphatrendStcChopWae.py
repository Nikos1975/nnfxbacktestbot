...
class NnfxVolumeMcginleyAlphatrendStcChopWae(IStrategy):
    """
    McGinley baseline + AlphaTrend proxy in MFI mode + STC proxy + CHOP gate + WAE decay exit.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["baseline"] = self._mcginley(close, period=10)
        dataframe["baseline_slope"] = dataframe["baseline"].diff(3)
        dataframe["alphatrend"] = self._alphatrend_proxy(dataframe, period=14, multiplier=1.0)
        dataframe["stc"] = self._stc_proxy(close)
        dataframe["chop"] = self._chop(dataframe, 14)
        wae_strength, wae_explosion = self._wae_proxy(dataframe)
        dataframe["wae_strength"] = wae_strength
        dataframe["wae_explosion"] = wae_explosion
        dataframe["baseline_signal"] = (close > dataframe["baseline"]) & (dataframe["baseline_slope"] > 0)
        dataframe["c1_signal"] = dataframe["alphatrend"] > dataframe["alphatrend"].shift(2)
        dataframe["c2_signal"] = dataframe["stc"] > 25
        dataframe["volume_filter"] = dataframe["chop"] < 55
        dataframe["exit_signal"] = (close < dataframe["baseline"]) | (dataframe["wae_strength"] < dataframe["wae_explosion"])
        return dataframe
