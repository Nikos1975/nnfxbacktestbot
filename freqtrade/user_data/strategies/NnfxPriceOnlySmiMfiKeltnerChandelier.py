...
class NnfxPriceOnlySmiMfiKeltnerChandelier(IStrategy):
    """
    Price-only EMA trend -> SMI-of-MFI proxy, with Keltner gate and Chandelier exit.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["baseline"] = close.ewm(span=200, adjust=False, min_periods=200).mean()
        dataframe["baseline_slope"] = dataframe["baseline"].diff(5)
        dataframe["smi_mfi"] = self._smi_mfi_proxy(dataframe)
        dataframe["keltner_outside"] = self._keltner_outside(dataframe)
        atr = self._atr_rma(dataframe, 22)
        dataframe["chandelier_long"] = dataframe["high"].rolling(22, min_periods=22).max() - 3.0 * atr
        dataframe["baseline_signal"] = (close > dataframe["baseline"]) & (dataframe["baseline_slope"] > 0)
        dataframe["c1_signal"] = close > dataframe["baseline"]
        dataframe["c2_signal"] = dataframe["smi_mfi"] > -40
        dataframe["volume_filter"] = dataframe["keltner_outside"]
        dataframe["exit_signal"] = (close < dataframe["chandelier_long"]) | (dataframe["smi_mfi"] < 0)
        return dataframe
