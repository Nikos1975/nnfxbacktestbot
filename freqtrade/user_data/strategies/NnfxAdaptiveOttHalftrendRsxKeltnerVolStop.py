...
class NnfxAdaptiveOttHalftrendRsxKeltnerVolStop(IStrategy):
    """
    OTT proxy + Adaptive HalfTrend proxy + RSX-on-JMA proxy + Keltner gate + Volatility Stop.
    ...
    minimal_roi = {"0": 0.10, "720": 0.05, "2160": 0.02}
    stoploss = -0.15
    trailing_stop = False
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        support, ott_line = self._ott_proxy(close, period=2, multiplier=1.4)
        dataframe["ott_support"] = support
        dataframe["ott_line"] = ott_line
        dataframe["halftrend"] = self._halftrend_proxy(dataframe, period=6)
        dataframe["rsx"] = self._rsx_proxy(close.ewm(span=14, adjust=False, min_periods=14).mean(), period=14)
        dataframe["keltner_outside"] = self._keltner_outside(dataframe)
        atr = self._atr_rma(dataframe, self.atr_period)
        dataframe["vol_stop"] = close.rolling(self.atr_period, min_periods=self.atr_period).max() - self.volstop_mult * atr
        dataframe["baseline_signal"] = dataframe["ott_support"] > dataframe["ott_line"]
        dataframe["c1_signal"] = close > dataframe["halftrend"]
        dataframe["c2_signal"] = dataframe["rsx"] > 50
        dataframe["volume_filter"] = dataframe["keltner_outside"]
        dataframe["exit_signal"] = (close < dataframe["vol_stop"]) | (dataframe["rsx"] < 45)
        return dataframe
